...
foreach($folder in $folders) {
    if(!(Test-path("$($folder.fullName)\AppData\Roaming\Microsoft\Windows\ServerManager"))) {
        New-Item -Path "$($folder.fullName)\AppData\Roaming\Microsoft\Windows\ServerManager" -ItemType Directory
    }
    $xml.Save("$($folder.fullName)\AppData\Roaming\Microsoft\Windows\ServerManager\ServerList.xml")
}
#>
